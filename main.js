...
function handleMouseMove(e) {
  ...
  // 更新光标位置显示
  if (gridX >= 0 && gridX < canvasSize && gridY >= 0 && gridY < canvasSize) {
    document.querySelector('#cursor-position span').textContent = `坐标: $ {
      gridX
    },
    $ {
      gridY
    ...
    document.querySelector('#cursor-position span').textContent = `坐标: 0,
    0`;
  }
...
function loadComponents() {
  ...
  for (const[category, comps] of Object.entries(components)) {
    ...
    comps.forEach(compData = >{
      ...
      if (images[compData.id]) {
        ...
        // 如果资源未加载，显示占位符
        compDiv.innerHTML = ` < div style = "width:30px;height:30px;background:#eee;display:flex;align-items:center;justify-content:center;color:#888;font-size:10px;" > $ {
          compData.id
        } < /div>`;
                    }
                    
                    / / 添加点击事件compDiv.addEventListener('click', (e) = >{
          e.stopPropagation();
      ...
      categoryDiv.appendChild(gridDiv); componentsList.appendChild(categoryDiv);
    }
  ...
  function updateStatusBar() {
    ...
    document.querySelector('#block-count span').textContent = `已放置: $ {
      placedCount
    }个组件`;
  }
    ...
    fileInput.addEventListener('change', () = >{
      if (fileInput.files.length) {
        document.getElementById('load-status').innerHTML = ` < i class = "fas fa-check-circle" > </i> 已选择文件: ${fileInput.files[0].name}`;
            ...
            / / 文件拖放事件fileDropArea.addEventListener('dragover', (e) = >{
          e.preventDefault();
        ...
        fileDropArea.addEventListener('drop', (e) = >{
          ...
          if (e.dataTransfer.files.length) {
            ...
            / / 阻止右键菜单document.addEventListener('contextmenu', (e) = >{
